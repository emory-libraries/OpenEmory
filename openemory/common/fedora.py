...
class DigitalObject(models.DigitalObject):
    ...
    def get_default_pid(self):
        ...
        if pidman is not None:
            ...
            pid = '%s:%s' % (settings.FEDORA_PIDSPACE, pidman.pid_token)
            target = reverse("publication:view", kwargs={'pid': pid})
            # reverse() encodes the PID_TOKEN, so unencode just that part
            target = target.replace(iri_to_uri(pidman.pid_token), pidman.pid_token)
            ...
            # Add full uri ARK to dc:identifier and MODS
            self.dc.content.identifier_list.append(ark)
            self.descMetadata.content.ark_uri = ark
            self.descMetadata.content.ark =  "ark:/%s/%s" % (naan, noid)
