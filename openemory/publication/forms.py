...
from openemory.publication.models import PublicationMods, \
    ...
    MODSCopyright, MODSAdminNote, SupplementalMaterial, BookMods,ReportMods, ChapterMods, ConferenceMods, PosterMods, PresentationMods
...
class ChapterModsEditForm(BaseXmlObjectForm):
    pages = SubformField(formclass=PartExtentEditForm, label='Page Range', required=False)
    class Meta:
        model = ChapterMods
        fields = ['pages']
...
class PresentationModsEditForm(BaseXmlObjectForm):
    presentation_place = forms.CharField(label='Presentation Place', widget=forms.TextInput(attrs={'class': 'text'}), required=False)
    class Meta:
...
class ChapterEditForm(PublicationModsEditForm):
    publication_place = forms.CharField(required=False, label='Publication Place')
    chapter = SubformField(formclass=ChapterModsEditForm)
    book = SubformField(formclass=BookModsEditForm)
    title_info = SubformField(formclass=ArticleModsTitleEditForm)
    authors = SubformField(formclass=AuthorNameForm)
    funders = SubformField(formclass=FundingGroupEditForm)
...
class ReportEditForm(PublicationModsEditForm):
    report = SubformField(formclass=ReportModsEditForm)
    title_info = SubformField(formclass=ArticleModsTitleEditForm)
    authors = SubformField(formclass=AuthorNameForm)
    funders = SubformField(formclass=FundingGroupEditForm)
    final_version = SubformField(formclass=FinalVersionForm)
...
class PosterEditForm(PublicationModsEditForm):
    poster = SubformField(formclass=PosterModsEditForm)
    title_info = SubformField(formclass=ArticleModsTitleEditForm)
    authors = SubformField(formclass=AuthorNameForm)
    funders = SubformField(formclass=FundingGroupEditForm)
    final_version = SubformField(formclass=FinalVersionForm)
    ...
    class Meta:
        model = PublicationMods
        fields = ['title_info','authors', 'publication_date', 'subjects',
                  'funders', 'poster', 'final_version', 'abstract', 'keywords',
                  'author_notes', 'language_code', 'copyright', 'admin_note', 'rights_research_date',
                  'supplemental_materials']


class PresentationEditForm(PublicationModsEditForm):
    presentation = SubformField(formclass=PresentationModsEditForm)
    title_info = SubformField(formclass=ArticleModsTitleEditForm)
    authors = SubformField(formclass=AuthorNameForm)
    funders = SubformField(formclass=FundingGroupEditForm)
    final_version = SubformField(formclass=FinalVersionForm)
    ...
    class Meta:
        model = PublicationMods
        fields = ['title_info','authors', 'publication_date', 'subjects',
                  'funders', 'presentation', 'final_version', 'abstract', 'keywords',
                  'author_notes', 'language_code', 'copyright', 'admin_note', 'rights_research_date',
                  'supplemental_materials']
